...
BinaryHeap.prototype.insert = function (value) {
	this._heap.push(value);
  	var index = this._heap.indexOf(value);

	while(index !== 0 ) {
	  	var parentIndex = Math.floor((index-1)/2);
	  	if (!this._compare(this._heap[parentIndex], this._heap[index])) {
	  		this.switchElems(parentIndex, index);
	  	}else {
	  		break;
	  	}
		  	index = this._heap.indexOf(value);
		  	console.log(index);
	  }
...
BinaryHeap.prototype.removeRoot = function () {
	var index = 0;
	this.switchElems(index, this._heap.length-1);
	this._heap.pop();

	while(this.hasChildren(index)) {
		var smallestIdx = this.smallestChild(index);

		if (this._compare(this._heap[smallestIdx], this._heap[index])) {
			this.switchElems(index, smallestIdx);
			index = smallestIdx;
		}else{
			break;
		}
...
BinaryHeap.prototype.switchElems =function(parentIndex, index) {
	var temp = this._heap[parentIndex];
	this._heap[parentIndex] = this._heap[index];
	this._heap[index] = temp;
}

BinaryHeap.prototype.smallestChild = function(index) {
	var firstChild = index * 2 + 1;
	var secondChild = index * 2 + 2;
	var smallest = secondChild;

	if (this._compare(this._heap[firstChild], this._heap[secondChild])) {
		smallest = firstChild;
	}
...
BinaryHeap.prototype.hasChildren = function(index) {
	var childrenIndices = [index * 2 + 1, index * 2 + 2];
	var hasChildren = false;

	if (childrenIndices[0] <= this._heap.length || childrenIndices[1] <= this._heap.length) {
		hasChildren = true;
	}
	return hasChildren;
}

var bn = new BinaryHeap();